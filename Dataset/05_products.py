...
# -*- coding: utf-8 -*-
"""
STEP 5: Create olist_products_dataset_vn.csv
"""
...
def main() -> None:
    ...
    ensure_dir(out_dir)
    rng = make_rng(args.seed, salt=5)

    counts = product_category_counts()

    rows = []
    pid = 1
    for cat_name, count in counts.items():
        pool = PRODUCT_POOL[cat_name]
        chosen = []
        while len(chosen) < count:
            chosen.append(pool[len(chosen) % len(pool)])

        for brand, name in chosen[:count]:
            rows.append(
                (
                    make_id("PRD", pid),
                    cat_name,
                    ...
                    name,
                    len(name),
                    int(rng.integers(109, 501)),
                    int(rng.integers(1, 6)),
                    int(rng.integers(187, 7998)),
                    int(rng.integers(6, 78)),
                    int(rng.integers(2, 59)),
                    int(rng.integers(6, 81)),
                )
            )
            pid += 1
    ...
    if len(df) != N_PRODUCTS:
        raise ValueError(f"Expected {N_PRODUCTS} products, got {len(df)}")

    write_csv(df, out_dir / "olist_products_dataset_vn.csv", args.force)
    print("OK: olist_products_dataset_vn.csv")
