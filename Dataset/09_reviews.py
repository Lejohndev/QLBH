...
# -*- coding: utf-8 -*-
"""
STEP 9: Create olist_order_reviews_dataset_vn.csv
...
def main() -> None:
    ...
    ensure_dir(out_dir)
    rng = make_rng(args.seed, salt=9)
    set_py_random(args.seed, salt=9)
    ...
    review_orders = order_ids[:N_REVIEWS]

    score_counts = {1: 429, 2: 427, 3: 1001, 4: 2122, 5: 2521}
    scores = []
    ...
    pos_msgs = [
        ...
        "Hàng giao nhanh, đóng gói cẩn thận.",
    ]
    neu_msgs = ["Giao hàng chậm, chưa hài lòng lắm."]
    neg_msgs = ["Sản phẩm không giống hình.", "Đóng gói sơ sài, bị móp méo."]

    start_r = datetime(2023, 1, 1, 0, 0, 0)
    end_r = datetime(2024, 12, 31, 23, 59, 59)
    created = random_datetimes(rng, start_r, end_r, N_REVIEWS)
    ans_days = rng.integers(0, 6, size=N_REVIEWS)  # 0..5 days

    rows = []
    for i, (oid, score, ct, add_days) in enumerate(zip(review_orders, scores, created, ans_days), start=1):
        if score >= 4:
            title = "Đánh giá tích cực"
            msg = random.choice(pos_msgs)
        elif score == 3:
            title = "Đánh giá trung bình"
            msg = random.choice(neu_msgs)
        else:
            title = "Đánh giá tiêu cực"
            msg = random.choice(neg_msgs)

        at = ct + timedelta(days=int(add_days))
        rows.append(
            (
                make_id("REV", i),
                oid,
    ...
    write_csv(df, out_dir / "olist_order_reviews_dataset_vn.csv", args.force)
    print("OK: olist_order_reviews_dataset_vn.csv")
