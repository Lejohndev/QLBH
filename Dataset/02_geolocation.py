...
# -*- coding: utf-8 -*-
"""
STEP 2: Create olist_geolocation_dataset_vn.csv
"""
...
def main() -> None:
    ...
    ensure_dir(out_dir)
    rng = make_rng(args.seed, salt=2)

    # zip 5 digits
    zips = rng.choice(range(10009, 91986), size=N_GEO, replace=False)
    city_idx = rng.choice(len(SELLER_GEO), size=N_GEO, replace=True)

    rows = []
    for z, idx in zip(zips, city_idx):
        base = SELLER_GEO[int(idx)]
        lat = float(base.lat + rng.normal(0, 0.05))
        lng = float(base.lng + rng.normal(0, 0.05))
        rows.append((int(z), lat, lng, base.city, base.state))
    ...
    write_csv(df, out_dir / "olist_geolocation_dataset_vn.csv", args.force)
    print("OK: olist_geolocation_dataset_vn.csv")
