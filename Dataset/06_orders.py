...
# -*- coding: utf-8 -*-
"""
STEP 6: Create olist_orders_dataset_vn_new.csv
Requires: olist_customers_dataset_vn_new.csv
"""
...
def main() -> None:
    ...
    ensure_dir(out_dir)
    rng = make_rng(args.seed, salt=6)

    cust_path = out_dir / "olist_customers_dataset_vn_new.csv"
    if not cust_path.exists():
        raise FileNotFoundError(f"Missing input: {cust_path}")

    customers_df = pd.read_csv(cust_path)
    cust_ids = customers_df["customer_id"].tolist()

    order_ids = [make_id("ORD", i) for i in range(1, N_ORDERS + 1)]

    # ensure every customer appears >= 1 order
    order_customers = cust_ids + rng.choice(cust_ids, size=N_ORDERS - len(cust_ids), replace=True).tolist()
    rng.shuffle(order_customers)

    start = datetime(2024, 1, 1, 8, 0, 0)
    end = datetime(2024, 1, 17, 23, 58, 0)

    total_min = int((end - start).total_seconds() // 60)
    offsets = rng.integers(0, total_min + 1, size=N_ORDERS)
    offsets.sort()  # make time increasing with order_id

    purchase_times = [start + timedelta(minutes=int(m)) for m in offsets]

    rows = []
    for i, oid in enumerate(order_ids):
        purchase = purchase_times[i]
        approved = purchase + timedelta(minutes=30)
        carrier = approved + timedelta(days=1)
        delivered = carrier + timedelta(days=2)
        estimated = purchase + timedelta(days=7)
    ...
    write_csv(df, out_dir / "olist_orders_dataset_vn_new.csv", args.force)
    print("OK: olist_orders_dataset_vn_new.csv")
