...
# -*- coding: utf-8 -*-
"""
STEP 3: Create olist_sellers_dataset_vn.csv from geolocation
Requires: olist_geolocation_dataset_vn.csv
"""
...
def main() -> None:
    ...
    ensure_dir(out_dir)
    rng = make_rng(args.seed, salt=3)

    geo_path = out_dir / "olist_geolocation_dataset_vn.csv"
    if not geo_path.exists():
        raise FileNotFoundError(f"Missing input: {geo_path}")

    geo_df = pd.read_csv(geo_path)
    seller_zips = rng.choice(geo_df["geolocation_zip_code_prefix"].values, size=N_SELLERS, replace=False)

    zip2 = geo_df.set_index("geolocation_zip_code_prefix")[["geolocation_city", "geolocation_state"]]

    rows = []
    for i, z in enumerate(seller_zips, start=1):
        city, state = zip2.loc[int(z)].tolist()
        rows.append((make_id("SEL", i), int(z), city, state))

    df = pd.DataFrame(rows, columns=["seller_id", "seller_zip_code_prefix", "seller_city", "seller_state"])
    write_csv(df, out_dir / "olist_sellers_dataset_vn.csv", args.force)
    print("OK: olist_sellers_dataset_vn.csv")
