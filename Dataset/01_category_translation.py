...
# -*- coding: utf-8 -*-
"""
STEP 1: Create product_category_name_translation_vn.csv
"""
...
def main() -> None:
    ...
    ap.add_argument("--all", action="store_true", help="Run all 9 steps (1->9)")
    ...
    write_csv(df, out_dir / "product_category_name_translation_vn.csv", args.force)
    print("OK: product_category_name_translation_vn.csv")
    ...
    if getattr(args, 'all', False):
        ...
        for s in scripts:
            cmd = [sys.executable, str(here / s), '--out_dir', str(args.out_dir), '--seed', str(args.seed)]
            if args.force:
                cmd.append('--force')
